...
function errorJson($msg){
	//print json_encode(array('maher error'=>$msg));
...
function delete($id) 
{
	...
	$login = query("SELECT * FROM login WHERE IdUser = '%d' ", $id);

	if (count($login['result']) == 0) 
	{
		//the username exists, return error to the iPhone app
		errorJson('User does not exists!');
	}

	//try to insert a new row in the "login" table with the given username and password
	$result = query("DELETE FROM iReport.login WHERE IdUser = '%d' ", $id);

	if (!$result['error']) {
		//Deletion is susccessfull, try to also directly login the new user
		//login($user, $pass);
		//just print out confirmation to the iPhone app
		//print json_encode(array('deletion'=>1));
		logout();
	} 
	else 
	{
		//for some database reason the registration is unsuccessfull
		errorJson('Deletion failed!');
	}

}


function updateRecord($user, $pass, $firstname, $lastname, $birthday, $address, $sex, $phonenumber, $email, $authorizationLevel, $activeRecord, $id) 
{

	$result = query("UPDATE  login SET
		firstname = '%s',
		lastname = '%s',
		birthday = '%s',
		address = '%s',
		...
		phonenumber = '%s',
		email = '%s',
		authorizationLevel = '%d' ,
		activeRecord = '%d'
	WHERE IdUser = '%d';
	", $firstname, $lastname, $birthday, $address, $sex, $phonenumber, $email, $authorizationLevel, $activeRecord, $id);
...
function login($user, $pass) {
	...
	$result = query("SELECT IdUser, username, firstname, lastname, phonenumber, address, email FROM login WHERE username='%s' AND pass='%s' limit 1",  $user, $pass);
	...
	if (count($result['result'])>0) 
	{
		...
		//This adds new item and value to the sql query result
		$result['result'][0]['status'] = $_POST['command'];
...
// 			if (move_uploaded_file($photoData['userfile']['tmp_name'], "upload/maher1.jpg")) {
				...
				errorJson('Upload on server problem '.$photoData['tmp_name']);
			};
...
function logout() 
{
	...
	print json_encode(array('logout'=>$_POST['command']));
